...
public class LoggingAspect {
    ...
    *   매개변수로 전달한 JoinPoint 는 현재 조인포인트의 메소드명, 인수값 등의
    *   정보들을 가져올 수 있다.
    * */
    ...
    *   또한 JoinPoint 는 반드시 첫번째 매개변수로 선언을 해야한다.
    *   */
    @AfterReturning(pointcut = "logPointcut()",returning = "result")
    public void logAfterReturning(JoinPoint joinPoint, Object result){
        System.out.println("After Returning result : "+result);
        /* result 라는 공간에 반환값이 들어있다. 따라서 결과값을 변셩할 수도 있다. */
        if(result != null && result instanceof Map){
            ((Map<Long,MemberDTO>) result).put(50L, new MemberDTO(50L, "변형"));
        }
    }
    /* 필기. 대상 메소드(joinpoint)를 실행하는 도중 예외가 발생하는 경우 공통의 기능 수행
    *   throwing 매개변수의 이름과 동일해야 된다.
    * */
    ...
    *  */
    ...
    public Object logAround(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        /* 필기. 원본 조인포인트를 호출한 쪽 or 다른 어드바이스가 다시 실행할 수 있도록 반환을 해준다. */
        return result;
    }
